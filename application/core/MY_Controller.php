...
class MY_Controller extends CI_Controller {
    ...
    private function interpolate_model() {
        if (file_exists(APPPATH)) {
            $interpolated_model = strtolower(get_class($this)) . '_model';

            if (file_exists(APPPATH . 'models/' . $interpolated_model . '.php')) {
                $this->interpolated_model = $interpolated_model;

                $this->load->model($interpolated_model);
            }
        }
    }
